        ...
        <nav id="main-menu" className="sticky top-0 z-50 relative flex flex-wrap items-center content-between py-3 px-4 bg-white/60 backdrop-blur-sm" role="banner">
            <div className="container flex flex-col lg:flex-row md:justify-between items-start lg:items-center">
                <section className="flex flex-row md:flex-col lg:flex-none justify-between w-full lg:w-auto">
                    <Link className="inline-block pt-1 pb-1 mr-4 text-lg whitespace-no-wrap flex flex-row space-x-1.5 items-center" href="/"><Image className={`h-10 w-auto`} src={logo} alt="logo" height="0" width="0" size="100vw" /><label className={`text-2xl font-bold`}>Subham Sarkar</label></Link>
                ...
                <section ref={navRef} className="hidden w-full lg:w-auto md:flex md:flex-row lg:place-content-end pt-4 lg:pt-0 ease-out">
                    <ul className="h-64 py-1.5 md:py-0 md:h-auto flex flex-col md:flex-row pl-0 mb-0 list-reset justify-between w-full space-y-4 md:space-y-0 md:space-x-5 overflow-auto text-lg">
                        <Link href={{ pathname: '/' }}>Home</Link>
                        <Link href={{ pathname: '/', hash: "features" }}>Features</Link>
                        <Link href={{ pathname: '/', hash: "services" }}>Services</Link>
                        <Link href={{ pathname: '/', hash: "about" }}>About</Link>
                        <Link href={{ pathname: '/', hash: "meet-team" }}>Team</Link>
                        <Link href={{ pathname: '/', hash: "get-in-touch" }}>Contact</Link>
                    </ul>
                </section>
            </div>
